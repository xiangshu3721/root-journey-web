...
function Home({ data, setView }: { data: JourneyData; setView: (view: View) => void }) {
  ...
  return <div className="page mvp-home"><div className="home-breadcrumb">寻根之旅 · 原生家庭考古</div><section className="home-assessment-full"><AssessmentResult assessment={assessment} onRestart={() => setView('assessment')} /></section><section className="home-flow"><span>探索路径</span><div><b>01</b>关系模式测试<i /></div><div><b>02</b>记录父母故事<i /></div><div><b>03</b>获得困惑洞察</div></section><section className="home-dilemma"><span>困惑洞察</span><h2>最近有什么事情，让你想不明白？</h2><p>系统会结合你的测试结果、父亲与母亲的真实材料，从原生家庭视角给出一次个性化理解。</p><button className="primary" onClick={() => setView('dilemma')}>写下我的困惑 <b>→</b></button></section><section className="home-parents"><div className="home-section-head"><span>我的父母</span><button className="text-button" onClick={() => setView('parents')}>查看全部档案 →</button></div><div>{(['mother', 'father'] as const).map((personId) => <button key={personId} onClick={() => setView('parents')}><i className={personId}>{personId === 'mother' ? '母' : '父'}</i><section><b>{personId === 'mother' ? '母亲档案' : '父亲档案'}</b><p>已记录 {rawCount(personId)} 段真实材料</p></section><em>继续记录 →</em></button>)}</div></section><section className="one-on-one-mvp"><div><span>觉塑 · 1v1 深度沟通</span><h2>如果你想有人陪你<br />把这些关系再往深处走。</h2><p>扫码联系觉塑。你可以带着已经记录的故事与洞察，进入更深入的梳理。</p></div><img src={wechatQr} alt="觉塑微信二维码" /></section><p className="local-note">当前资料仅保存在这台设备的浏览器中。更换设备、浏览器或清除浏览器数据后，资料可能无法恢复。</p></div>;
}
