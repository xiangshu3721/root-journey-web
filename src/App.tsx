...
export default function App() {
  ...
  return <div className="app-shell dashboard-shell"><aside><Brand /><nav>{nav.map(([key, title, icon]) => <button key={key} className={view === key ? 'selected' : ''} onClick={() => setView(key)}><i>{icon}</i>{title}</button>)}</nav><button className="device-clear" onClick={() => void clearAll()}>清除本机资料</button><button className="theme-toggle" onClick={() => setTheme((current) => current === 'light' ? 'dark' : 'light')}>{theme === 'dark' ? '切换日间' : '切换夜间'}</button></aside><main>{view === 'home' && <Home data={data} persist={persist} setView={setView} onRestart={() => { setRetest(true); setView('assessment'); }} />} {view === 'parents' && <ParentArchive data={data} persist={persist} toast={toast} />} {view === 'dilemma' && <DilemmaInsight data={data} persist={persist} toast={toast} />} {view === 'assessment' && <section className="page"><FamilyPatternTest assessment={retest ? undefined : data.familyAssessment} startAtTest={retest} onComplete={async (assessment) => { setRetest(false); await saveAssessment(assessment); }} onContinueParents={() => setView('parents')} onExit={() => { setRetest(false); setView('home'); }} /></section>}</main>{notice && <div className="toast">{notice}</div>}<div className="mobile-nav">{nav.map(([key, title, icon]) => <button key={key} className={view === key ? 'selected' : ''} onClick={() => setView(key)}><i>{icon}</i>{title}</button>)}</div></div>;
}

function Brand() { return <div className="brand"><b>觉塑</b><span>自我重塑成长系统</span></div>; }

function Home({ data, persist: _persist, setView, onRestart }: { data: JourneyData; persist: (next: JourneyData) => Promise<void>; setView: (view: View) => void; onRestart: () => void }) {
