...
export function FamilyPatternTest({ data }: { data: JourneyData }) {
  ...
  if (mode === 'summary') return <section className="pattern-test pattern-summary"><div><span>原生家庭关系模式</span><h2>你的关系模式缩略图</h2><p>当前最值得留意：{ranked[0]?.name || '继续完成探索'}。</p><button className="text-button" onClick={() => setMode('result')}>查看详细结果　›</button></div><div className="mini-radar"><Radar scores={scores}/></div><div className="summary-scores">{scores.map((score, index) => <span key={dimensions[index].name}>{dimensions[index].name}<b>{score}</b></span>)}</div></section>;
