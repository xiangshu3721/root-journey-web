...
export function ParentArchive({ data, persist, toast }: { data: JourneyData; persist: (next: JourneyData) => Promise<void>; toast: (message: string) => void }) {
  ...
  return <div className="page parent-archive">
    ...
    {showBasics && <section className="parent-basics"><div className="basic-form"><h2>{personId === 'mother' ? '母亲的基础信息' : '父亲的基础信息'}</h2><div>
      ...
      <label>出生日期<ChineseDatePicker value={person.birthDate} onChange={(value) => update('birthDate', value)} /></label>
...
function ChineseDatePicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const normalized = normalizeDate(value);
  const [year = '', month = '', day = ''] = normalized ? normalized.split('-') : [];
  ...
  function select(nextYear: string, nextMonth: string, nextDay: string) { onChange(nextYear && nextMonth && nextDay ? `${nextYear}-${nextMonth.padStart(2, '0')}-${nextDay.padStart(2, '0')}` : ''); }
  return <span className="date-picker"><select value={year} onChange={(event) => select(event.target.value, month, day)}><option value="">选择年份</option>{years.map((item) => <option key={item} value={item}>{item} 年</option>)}</select><select value={month} disabled={!year} onChange={(event) => select(year, event.target.value, day)}><option value="">选择月份</option>{Array.from({ length: 12 }, (_, index) => index + 1).map((item) => <option key={item} value={item}>{item} 月</option>)}</select><select value={day} disabled={!month} onChange={(event) => select(year, month, event.target.value)}><option value="">选择日期</option>{Array.from({ length: days }, (_, index) => index + 1).map((item) => <option key={item} value={item}>{item} 日</option>)}</select></span>;
}
...
function AreaPicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return <span className="area-picker"><select value={provinceCode} onChange={(event) => save(areaList.province_list[event.target.value] || '')}><option value="">选择省份</option>{provinces.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select><select value={cityCode} disabled={!provinceCode} onChange={(event) => save(provinceName, areaList.city_list[event.target.value] || '')}><option value="">选择城市</option>{cities.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select><select value={countyCode} disabled={!cityCode} onChange={(event) => save(provinceName, cityName, areaList.county_list[event.target.value] || '')}><option value="">选择区 / 县</option>{counties.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></span>;
}
