...
import { deriveParentHighValueSignals } from '../lib/parentSignals';
...
export function ParentArchive({ data, persist, toast }: { data: JourneyData; persist: (next: JourneyData) => Promise<void>; toast: (message: string) => void }) {
  ...
  async function organize(nextMaterials = data.materials) { const relevant = nextMaterials.filter((item) => item.personId === personId && !item.isRaw); const output = await api.parentPortrait(personId, relevant); const parentLabel = personId === 'mother' ? '母亲' : '父亲'; const nextPortrait: Insight | undefined = output ? { id: crypto.randomUUID(), kind: 'portrait', status: 'confirmed', title: `${parentLabel} · 结构化档案`, body: '基于已录入材料形成的结构化档案。', portraitSections: output.sections, portraitSummary: output.summary, portraitExtras: output.extras, portraitVersion: 3, sourceIds: relevant.map((item) => item.id), materialSignature: relevant.map((item) => item.id).sort().join('|') } : undefined; const highValue = deriveParentHighValueSignals(nextMaterials.filter((item) => item.personId === personId)); await persist({ ...data, materials: nextMaterials, parentHighValueSignals: { ...data.parentHighValueSignals, [personId]: highValue }, insights: nextPortrait ? [nextPortrait, ...data.insights.filter((item) => !(item.kind === 'portrait' && item.title.startsWith(`${parentLabel} ·`)))] : data.insights }); return Boolean(nextPortrait) || Boolean(highValue.frequentQuotes.length || highValue.shapingChains.length || highValue.implicitRules.length || highValue.positiveLegacy.length); }
