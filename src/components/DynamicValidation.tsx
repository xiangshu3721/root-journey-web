...
export function DynamicValidation({ data, persist, toast }: { data: JourneyData; persist: (next: JourneyData) => Promise<void>; toast: (message: string) => void }) {
  ...
  const pending = useMemo(() => data.insights.filter((item) => (item.kind === 'summary' || item.kind === 'hypothesis') && item.status === 'pending' && isActionableInsight(item.body) && !hidden.includes(item.id)), [data.insights, hidden]);
