...
export interface DilemmaModule { id: 'agency' | 'emotion' | 'relationship' | 'value'; name: string; association: number; role: '核心课题' | '伴随课题' | '辅助课题' | '深层课题'; direction: string; }
export interface DilemmaAnalysis { coreInsight: string; mode: { event: string; interpretation: string; emotion: string; need: string; response: string; result: string; belief: string; }; formation: { connection: string; clues: Array<{ label: string; text: string }> }; modules: DilemmaModule[]; newChoice: string[]; growthFocus: string; }
export interface Insight { id: string; title: string; body: string; sourceIds: string[]; status: Approval; kind: 'summary' | 'hypothesis' | 'dilemma' | 'portrait'; dilemma?: DilemmaAnalysis; confidence?: '低' | '中' | '高'; portraitSections?: Record<string, string>; portraitSummary?: string; portraitExtras?: Record<string, string>; portraitVersion?: number; materialSignature?: string; }
export interface InnerRole { id: 'innerFather' | 'innerMother'; name: string; avatar: string; trait: string; basedOn: PersonId[]; }
export interface FamilyPatternAssessment { answers: Array<number | null>; scores: number[]; completedAt: string; familyClimate?: Array<number | null>; attributions?: Record<string, string>; openAnswers?: Record<string, string>; }
